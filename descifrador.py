...
def descifrar_codice():
    llave = "" # Aquí se irá armando la contraseña final
    
    # 1. Traemos todo el historial de una sola vez
    ...
    for linea in lineas_log:
        ...
        es_merge = len(partes) > 1 # Si tiene más de un padre, es un commit de merge
        ...
        # 3. EVALUACIÓN: Convertimos los primeros 6 caracteres del hash a número decimal 
        val_decimal = int(c_hash[:6], 16)
        
        # Contamos cuántos números (0-9) y cuántas letras (a-f) tiene el hash completo 
        cant_numeros = sum(1 for char in c_hash if char.isdigit())
        cant_letras = sum(1 for char in c_hash if char.lower() in 'abcdef')
        
        # 4. MÁQUINA DE ESTADOS: Aplicamos el cifrado según si el decimal es Par o Impar 
        if val_decimal % 2 == 0:
            # CASO PAR: Cifrado César al primer carácter usando la cantidad de números 
            ...
            # CASO IMPAR: Al valor ASCII del último carácter le sumamos la cantidad de letras (a-f) 
            letra_final = contenido[-1]
            caracter_final = chr(ord(letra_final) + cant_letras)
        
        # Agregamos el carácter descubierto a nuestra Llave 
        llave += caracter_final
        
        # 5. MUTACIÓN FINAL: Si este commit era un Merge, invertimos la Llave de inmediato
        if es_merge:
            llave = llave[::-1]
